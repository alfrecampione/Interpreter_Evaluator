...
public class Interpreter
{
    ...
    public string Parse(string function)
    {
        try
        {
            ...
            for (int i = 0; i < subsArray.Length; i++)
            {
                ...
                if (i != 0) subsArray[i] = string.Format(subsArray[i], subsArray[..i]);
            }
            //Agregar las susticiones a la función original
            function = string.Format(function, subsArray);
            //Eliminar las susticiones de la lista
            substitutions.Clear();
            IDs.Sort();
        }
        catch
    ...
    private static (string lhs, string? rhs) GetExpressions(string s, out string op)
    {
        ...
        {
            int i = s.IndexOf(oper, StringComparison.Ordinal);
            if (i >= 0)
            {
                op = oper;
                return (s[..i], s[(i + 1)..]);
    ...
    public Expression GetExpression(string function)
    {
        ...
        if (function.All(x => x != '('))
        {
            ...
            if (double.TryParse(function, out var number))
